...
class VLMEventSubscriber implements EventSubscriberInterface {
  ...
  /**
   * Filter a views AJAX response when the Load More pager is set.
   *
   * Remove the scrollTop and viewsScrollTop command and
   * add in a viewsLoadMoreAppend AJAX command.
   *
    ...
    if ($response instanceof ViewAjaxResponse) {
      ...
      if ($pagerPlugin->getPluginId() == 'load_more' && $pagerPlugin->getCurrentPage() > 0) {
        ...
        foreach ($commands as $key => $command) {
          // Remove 'scrollTop' and 'viewsScrollTop' command,
          // as this behavior is unnecessary.
          if (in_array($command['command'], ['scrollTop', 'viewsScrollTop'])) {
            unset($commands[$key]);
          }
          // The replace should be the only insert command,
          // but just in case, we'll make sure.
          elseif ($command['command'] == 'insert' && $command['method'] == 'replaceWith' && $command['selector'] == '.js-view-dom-id-' . $view->dom_id) {
            ...
            if ($stylePlugin->getPluginId() == 'html_list' && in_array($stylePlugin->options['type'], ['ul', 'ol'])) {
            ...
            elseif ($stylePlugin->getPluginId() == 'table') {
