...
class LoadMore extends Full {
  ...
  public function summaryTitle() {
    if (!empty($this->options['offset'])) {
      return $this->formatPlural(
        ...
        'Load more pager, @count item, skip @skip',
        'Load more pager, @count items, skip @skip',
        [
    ...
    return $this->formatPlural(
      $this->options['items_per_page'],
      'Load more pager, @count item',
      'Load more pager, @count items',
      [
        '@count' => $this->options['items_per_page'],
      ]
    );
  ...
  protected function defineOptions() {
    ...
    $options['more_button_text'] = ['default' => $this->t('Load more')];
    $options['end_text'] = ['default' => ''];

    $options['advanced']['contains']['content_selector'] = ['default' => ''];
    $options['advanced']['contains']['pager_selector'] = ['default' => ''];

    $options['effects']['contains']['type'] = ['default' => ''];
    $options['effects']['contains']['speed'] = ['default' => ''];
  ...
  public function buildOptionsForm(&$form, FormStateInterface $form_state) {
    ...
    // Option for users to specify the text shown when
    ...
    // Option to specify the content_selector,
    // which is the wrapping div for views rows.
    // This allows the JS to both find new rows on next pages and know
    // where to put them in the page.
    $form['advanced']['content_selector'] = [
      '#type' => 'textfield',
      '#title' => $this->t('Content selector'),
      '#description' => $this->t('jQuery selector for the rows wrapper, relative to the view container.  Use when overriding the views markup.  Note that Views Load More requires a wrapping element for the rows.  Unless specified, Views Load More will use <strong><code>@content_selector</code></strong>.', ['@content_selector' => LoadMore::DEFAULT_CONTENT_SELECTOR]),
    ...
    // view container.
    $form['advanced']['pager_selector'] = [
      '#type' => 'textfield',
      '#title' => $this->t('Pager selector'),
      '#description' => $this->t('jQuery selector for the pager, relative to the view container.  Use when overriding the pager markup so that Views Load More knows where to find and how to replace the pager.  Unless specified, Views Load More will use <strong><code>@pager_selector</code></strong>.', ['@pager_selector' => LoadMore::DEFAULT_PAGER_SELECTOR]),
    ...
    $form['effects']['speed'] = [
      ...
      '#states' => [
        'visible' => [
          ['#edit-pager-options-effects-type' => ['value' => 'fade']],
          ['#edit-pager-options-effects-type' => ['value' => 'slide']],
        ],
      ],
